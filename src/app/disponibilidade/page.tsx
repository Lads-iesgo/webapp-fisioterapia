...
export default function Disponibilidade() {
	...
	function handleDateClick(arg: { date: Date; dateStr: string }) {
		...
		// Procura se existe algum evento de background (indisponibilidade) neste dia
		const indisponibilidadeEncontrada = events.find((e) => {
			...
			return e.extendedProps?.status === "indisponivel" && eventDateStr === clickedDate;
		});

		// Se encontrou indisponibilidade, abrimos o modal de ação (se tiver permissão)
		if (indisponibilidadeEncontrada) {
			if (!temPermissaoIndisponibilidade) {
				showNotification("warning", "Este dia está bloqueado.");
				return;
			}
			setActionModalMode("unavailable");
			setSelectedUnavailableEventId(String(indisponibilidadeEncontrada.id));
	...
	// Efeito para transformar as consultas em eventos do FullCalendar
	useEffect(() => {
		// 1. Primeiro, contamos quantas consultas temos por dia
		const contagemDias = consulta.reduce((acc, item) => {
			const data = typeof item.data_consulta === "string"
		...
		const eventos: EventInput[] = consulta.map((item) => {
			...
			const fisioterapeutaNome = fisioterapeuta?.nome_completo ?? "Aluno não informado";

			// Verifica se este dia tem apenas 1 consulta
			const isSingle = contagemDias[data] === 1;
			...
			return {
				...
				extendedProps: {
					...
					isSingle: isSingle, // Adicionamos a flag aqui!
				},
			};
		});

		// 3. Mapeia as indisponibilidades (AGORA COM BACKGROUND)
	...
	return (
		<RouteGuard>
			...
			<div
				className={`fixed top-24 right-8 max-w-sm p-4 rounded-md shadow-lg transition-all duration-300 ${notification.show
...
if (display === "background" || extendedProps.status === "indisponivel") {
    ...
    ">
                  ...
                  <div class="bg-white border border-gray-200 rounded p-2 shadow-lg text-sm">
                    <p><strong>Paciente:</strong> ${info.event.extendedProps.pacienteNome || "Não informado"
									}</p>
                    <p><strong>Aluno:</strong> ${info.event.extendedProps.fisioterapeutaNome || "Não informado"
									}</p>
                    <p><strong>Horário:</strong> ${info.event.extendedProps.horario || "Não informado"
									}</p>
                  </div>
                `;
										...
										<div className='mt-5 sm:mt-6 flex flex-col space-y-3'>
											{actionModalMode === "default" ? (
												<>
													<button
														type='button'
														className='inline-flex w-full justify-center rounded-md bg-blue-950 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-950 focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-blue-950'
														onClick={() => {
															setShowActionModal(false);
															setShowModal(true);
														}}
							...
							<div className='flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0'>
                              ...
                              fisioterapeutas e dos horários cadastrados */}
															<Select
																...
																required
																className={`w-full rounded-md border ${formErrors.paciente_id
																		? "border-red-500"
																		: "border-gray-300"
														...
														<div className='w-full'>
															...
															<Select
																...
																required
																className={`w-full rounded-md border ${formErrors.fisioterapeuta_id
																		? "border-red-500"
																		: "border-gray-300"
																	} px-4 py-3 text-base select-custom`}
															>
														...
														<div className='w-full'>
															...
															<Select
																...
																required
																className={`w-full rounded-md border ${formErrors.horario_id
																		? "border-red-500"
																		: "border-gray-300"
																	} px-4 py-3 text-base select-custom`}
															>
